...
const selected_consumer_id = ref(props.data?.consumer_id??'');
const transport = ref(props.data?.transport??0);
const vat = ref(props.data?.vat??0);
const laboratory_no = ref(props.data?.laboratory_no??'');
const paid_amount = ref(props.data?.paid_amount??0);
...
if (props.data && props.data.sample_data && props.data.sample_data.length > 0) {
    samples.value = props.data.sample_data.map(s => {
        ...
        const chemical = 
            ...
            testMap['hardness'] && testMap['iron'] && testMap['sulphate'] &&
            testMap['calcium'] && testMap['manganese'] || false;

        return {
            id: s.id,
            reference_number: s.reference_number,
            source: s.source,
            sample_locations: s.sample_locations,
            quantity: s.quantity,
            ...
            chemical,
            // Sub-tests
            colour: testMap['colour'] || false,
            turbidity: testMap['turbidity'] || false,
            coliform: testMap['coliform'] || false,
            coli: testMap['coli'] || false,
            ph: testMap['ph'] || false,
            electrical: testMap['electrical'] || false,
            chloride: testMap['chloride'] || false,
            alkalinity: testMap['alkalinity'] || false,
            nitrate: testMap['nitrate'] || false,
            nitrite: testMap['nitrite'] || false,
            fluoride: testMap['fluoride'] || false,
            phosphate: testMap['phosphate'] || false,
            dissolvedSolid: testMap['dissolvedSolid'] || false,
            hardness: testMap['hardness'] || false,
            iron: testMap['iron'] || false,
            sulphate: testMap['sulphate'] || false,
            calcium: testMap['calcium'] || false,
            manganese: testMap['manganese'] || false,
        };
    });
} else {
    samples.value = [
...
async function handleSubmit() {
    ...
    store('consumer.samples.update', formData);
}
...
<template>
    <AdminLayout>
        ...
        <Head title="consumer-sample-edit" />

        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h4 class="m-0">CONSUMER SAMPLE EDIT</h4>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a :href="route('con.sample.index')">Consumer Sample</a>
                            </li>
                            <li class="breadcrumb-item active">Consumer Sample Edit</li>
                        </ol>
                    </div>
                </div>
            </div>
                                    ...
                                    <div class="row mt-3">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            <label for="paid_amount">Total Payment
                                                Amount</label>
                                            <input type="number" min="0" step="0.01" class="form-control" id="paid_amount" required
